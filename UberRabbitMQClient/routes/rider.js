...
function checkRiderLogin(req, res) {
	...
	mq_client.make_request('login_rider_queue', msg_payload, function(err,
			results) {
		if (err) {
			throw err;
		} else {
...
function getRiderDetails(req, res) {
	console.log("In Get Rider Details" + req.param("user"));
	...
	mq_client.make_request('get_rider_queue', msg_payload, function(err,
			results) {
		if (err) {
			console.log(err);
		} else {
