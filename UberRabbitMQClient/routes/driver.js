	...
	mq_client.make_request('login_Driver_queue', msg_payload,
		...
		if (err) {
			...
			if (results.code == 200) {
				...
				console.log("password printing \n");
				console.log("pwd"+password+"hash"+results.user.password);
				console.log("password printing after check \n");
				if(bcrypt.compareSync(password, results.user.password) == true) {
					...
					console.log("after data from mongo \n");
					console.log(results.user.firstname);
					console.log(results.user.lastname);
					console.log("preparing JSON responses\n");
					json_responses = {
							"statusCode" : results.code,
							"email" : email,
...
exports.checkDrivers = function(req,res){
	...
	console.log("inside checkDrivers email: "+email+" password: "+password);
	var json_responses;
